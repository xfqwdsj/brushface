...
class FaceNetBaseClient(KerasRecognizer):
    input_shape = (160, 160)


class FaceNet128dClient(FaceNetBaseClient):
    name = "FaceNet-128d"
    output_shape = 128
    ...
    def __init__(
        ...
        threshold: Threshold = MappingProxyType(
            {
                CosineDistance: 0.40,
                EuclideanDistance: 10,
                EuclideanL2Distance: 0.80,
            }
        ),
    ):
    ...
    def build_model(
        ...
        model = inception_res_net_v2(dimension=128)
        model.load_weights(get_weights(url, logger))
        return model


class FaceNet512dClient(FaceNetBaseClient):
    name = "FaceNet-512d"
    output_shape = 512
    ...
    def __init__(
        ...
        threshold: Threshold = MappingProxyType(
            {
                CosineDistance: 0.30,
                EuclideanDistance: 23.56,
                EuclideanL2Distance: 1.04,
            }
        ),
    ):
        self.model = self.build_model()
    ...
    def build_model(
        ...
        model = inception_res_net_v2(dimension=512)
        model.load_weights(get_weights(url, logger))
        return model
...
def inception_res_net_v2(dimension: int = 128):
    """
    Constructs the Inception-ResNet-v2 model.

    Args:
        dimension: The dimension of the embedding layer.
    Returns:
        The Inception-ResNet-v2 model.
    """
    ...
    up = Lambda(scaling, output_shape=shape(up)[1:], arguments={"scale": 0.17})(up)
    x = add([x, up])
    ...
    up = Lambda(scaling, output_shape=shape(up)[1:], arguments={"scale": 0.1})(up)
    x = add([x, up])
    ...
    up = Lambda(scaling, output_shape=shape(up)[1:], arguments={"scale": 0.2})(up)
    x = add([x, up])
    ...
    up = Lambda(scaling, output_shape=shape(up)[1:], arguments={"scale": 1})(up)
    x = add([x, up])
